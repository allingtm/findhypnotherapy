...
import { exchangeCodeForTokens, saveMicrosoftTokens } from '@/lib/calendar/microsoft';
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL(`/dashboard/availability?error=oauth_denied&message=${encodeURIComponent(errorDescription || error)}`, baseUrl)
    );
  }
  ...
  try {
    ...
    await saveMicrosoftTokens(userId, tokens);

    // Clear OAuth cookies and redirect to success
    const response = NextResponse.redirect(
      new URL('/dashboard/availability?calendar=microsoft&connected=true', baseUrl)
    );
